...
test.describe('Admin Panel', () => {
  ...
  test('can navigate to list view', async () => {
    const url = 'http://localhost:3000/admin/collections/users'
    await page.goto(url)
    await expect(page).toHaveURL((currentURL) => currentURL.pathname === new URL(url).pathname)
  ...
  test('FAQ を管理画面から作成して保存できる', async () => {
    ...
    await page.goto('http://localhost:3000/admin/collections/faq/create')
    ...
    await page.goto('http://localhost:3000/admin/collections/faq')
  ...
  test('サイト名の保存内容が公開ヘッダーとフッターへ反映される', async () => {
    ...
    const marker = `${originalSiteName} [QA-${Date.now()}]`
