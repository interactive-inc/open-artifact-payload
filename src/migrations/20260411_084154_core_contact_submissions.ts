...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`contact_submissions_id\` integer REFERENCES contact_submissions(id);`,
  )
  await db.run(
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "news_id", "faq_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "news_id", "faq_id" FROM \`payload_locked_documents_rels\`;`,
  )
